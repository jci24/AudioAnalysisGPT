...
public static class EvidencePackageBuilder
{
            ...
            if (summaryElement.TryGetProperty("dominantPeaks", out var peaksArray))
            {
                ...
                foreach (var peak in peaksArray.EnumerateArray())
                {
                    var freqHz = peak.TryGetProperty("frequencyHz", out var freqEl)
                        ? freqEl.GetDouble()
                        : 0.0;
                    var magDb = peak.TryGetProperty("magnitudeDb", out var magEl)
                        ? magEl.GetDouble()
                        : 0.0;
            ...
            if (summaryElement.TryGetProperty("highestBands", out var bandsArray))
            {
                ...
                foreach (var band in bandsArray.EnumerateArray())
                {
                    ...
                    var levelDb = band.TryGetProperty("levelDb", out var levelEl)
                        ? levelEl.GetDouble()
                        : 0.0;
                    var label = band.TryGetProperty("label", out var labelEl)
                        ? labelEl.GetString()
                        : "";
    ...
    {
        var fileResults =
            new List<(string FileId, double Loudness, double Sharpness, double Roughness)>();
        ...
        foreach (var fileResult in resultsArray.EnumerateArray())
        {
            ...
            if (
                !fileResult.TryGetProperty("loudness", out var loudnessEl)
            ...
            if (
                !fileResult.TryGetProperty("sharpness", out var sharpnessEl)
            ...
            if (
                !fileResult.TryGetProperty("roughness", out var roughnessEl)
            ...
            fileResults.Add(
                (
                    fileId,
                    loudnessValEl.GetDouble(),
                    sharpnessValEl.GetDouble(),
                    roughnessValEl.GetDouble()
                )
            );
        }
